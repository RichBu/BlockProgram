...
export function getTestCompileOptsAsync(packageFiles: pxt.Map<string> = { [pxt.MAIN_TS]: "// no main" }, dependencies: string[] = [], includeCommon = false): Promise<pxtc.CompileOptions> {
...
export function ts2pyAsync(tsInput: string, dependency: string, includeCommon: boolean, caseName: string): Promise<string> {
    ...
    return getTestCompileOptsAsync({ [pxt.MAIN_TS]: tsInput }, deps, includeCommon)
        .then(opts => {
    ...
    return getTestCompileOptsAsync({ [pxt.MAIN_PY]: pyInput, [pxt.MAIN_TS]: "// no main" }, deps, includeCommon)
        .then(opts => {
            ...
            if (success || allowErrors) {
                return {
                    python: pyInput,
                    ts: opts.fileSystem[pxt.MAIN_TS],
                    diagnostics
                };
            }
            else {
                let partialOutput = outfiles[pxt.MAIN_TS]
...
export function stsAsync(tsMain: string, dependency?: string, includeCommon = false): Promise<pxtc.CompileResult> {
    const deps = [...(dependency ? [dependency] : []), ...(!includeCommon ? ["bare"] : [])]
    return getTestCompileOptsAsync({ [pxt.MAIN_TS]: tsMain }, deps, includeCommon)
        .then(opts => {
